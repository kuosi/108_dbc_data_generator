...
#default configuration and data directory. Shall be mounted (with docker volumes) when the program is run in a docker container
config_dir = "../config/"
data_dir = "../data/"

#Load the configuration file
f = open(config_dir + 'configuration.json')
config = json.load(f)
f.close()
...
dbc_file_name = config["dbc"]["file"] #DBC file name, that shall be used for generating CAN messages. The dbc file name shall be available in the configuration directory.
dbc_messages = config["dbc"]["IDs"] #Semi-column separated CAN IDs

#Load the dbc file
db = cantools.database.load_file(config_dir + dbc_file_name)
